...
import { fetchPropertyById, getApiUrl } from '@/lib/api';
...
export default function EditPropertyPage() {
    ...
    useEffect(() => {
        const loadProperty = async () => {
            try {
                ...
                // Permission check (also enforced on backend)
                if (user && user.role !== 'ADMIN' && property.ownerId !== user.id) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${getApiUrl()}/api/users/properties/${params?.id}`, {
                method: 'PUT',
    ...
    if (fetching) {
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="w-12 h-12 border-4 border-[var(--marketing-green)] border-t-transparent rounded-full animate-spin"></div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50 pt-24 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-[var(--forest-green)] font-playfair mb-2">Edit Listing</h1>
                    <p className="text-[var(--text-secondary)]">Update the details of your property.</p>
                </div>
